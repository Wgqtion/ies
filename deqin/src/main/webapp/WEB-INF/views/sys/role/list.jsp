	...
	<table class="table" width="100%" layoutH="138">
		<thead>
			<tr>
				...
				<th width="150" align="center"  <vsc:orderField name="displayName"/>>角色名称</th>
				<th width="150" <vsc:orderField name="name"/>>授权分配</th>
				<th width="80">操作</th>
			</tr>
		</thead>
		<tbody>
			<c:forEach items="${page.content}" var="item" varStatus="index">
				<tr target="sid" rel="${item.id}">
					...
					<td>
						<a href="${ctx}/sys/role/view/${item.id}" target="dialog" title="查看">${item.name}</a>
					</td>
					<td> 
					    <vsc:fetchElementPropertyToString propertyName="displayName" list="${item.authorityList}"></vsc:fetchElementPropertyToString>
					</td>
					<td> 

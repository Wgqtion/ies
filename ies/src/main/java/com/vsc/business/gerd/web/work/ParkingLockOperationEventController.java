...
public class ParkingLockOperationEventController extends BaseController {
	...
	public ModelAndView exportList(HttpServletRequest request){
		Map<String, Object> searchParams = this.getSearchRequest();
		List<ParkingLockOperationEvent> list=parkingLockOperationEventService.findAll(searchParams, this.getSortOrderBy(), this.getSortOrderDesc());
	...
	public String createForm(Model model) {
		model.addAttribute("vm", new ParkingLockOperationEvent());
		model.addAttribute("action", BaseController.CREATE);
		return PATH_EDIT;
	}

	@RequestMapping(value =  BaseController.CREATE, method = RequestMethod.POST)
	public ModelAndView create(@Valid ParkingLockOperationEvent parkingLockOperationEvent) {
		parkingLockOperationEventService.save(parkingLockOperationEvent);		 
		return this.ajaxDoneSuccess("创建成功");
	}

	@RequestMapping(value =  BaseController.UPDATE+"/{id}", method = RequestMethod.GET)
	public String updateForm(@PathVariable("id") Long id, Model model) {
		model.addAttribute("vm", parkingLockOperationEventService.getObjectById(id));
	...
	public ModelAndView update(@Valid @ModelAttribute("preloadModel") ParkingLockOperationEvent parkingLockOperationEvent) {
		parkingLockOperationEventService.save(parkingLockOperationEvent);		
	...
	public ModelAndView delete(@PathVariable("id") Long id) {
		parkingLockOperationEventService.deleteById(id);		 
		return this.ajaxDoneSuccess("删除成功");
	}
	
	@RequestMapping(value = BaseController.DELETE,method = RequestMethod.POST)
	public ModelAndView deleteBatch(@RequestParam Long[] ids) {
		parkingLockOperationEventService.deleteByIds(ids);		 
		return this.ajaxDoneSuccess("删除成功");
	}

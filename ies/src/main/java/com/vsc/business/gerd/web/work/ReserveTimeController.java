...
public class ReserveTimeController extends BaseController {
	...
	public String createForm(Model model) throws Exception {
		...
		model.addAttribute("vm",reserveTime);
	...
	public ModelAndView create(@Valid ReserveTime reserveTime) throws Exception {
		reserveTimeService.save(reserveTime);
		return this.ajaxDoneSuccess("创建成功");
	}

	@RequestMapping(value = BaseController.UPDATE + "/{id}", method = RequestMethod.GET)
	public String updateForm(@PathVariable("id") java.lang.Long id, Model model) throws Exception {
		ReserveTime reserveTime= reserveTimeService.getObjectById(id);
		model.addAttribute("vm",reserveTime);
		model.addAttribute("action", BaseController.UPDATE);
		model.addAttribute("parkingLot",reserveTime.getParkingLot());
		model.addAttribute("parkingLotTree",this.parkingLotService.findTree());
		return PATH_EDIT;
	}

	@RequestMapping(value = BaseController.UPDATE, method = RequestMethod.POST)
	public ModelAndView update(@Valid @ModelAttribute("preloadModel") ReserveTime reserveTime) throws Exception {
		reserveTimeService.save(reserveTime);

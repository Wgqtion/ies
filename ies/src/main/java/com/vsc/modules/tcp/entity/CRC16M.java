...
public class CRC16M {
	...
	public void update(int[] puchMsg, int usDataLen) {
	...
	public static boolean checkCRC(String str,int len) {
		int[] data = HexUtils.HexString2Int(str);

		CRC16M crc16 = new CRC16M();
		crc16.update(data, len);
		int ri = crc16.getValue();
		int crcH = (ri >> 8) & 0xff;
		int crcL = (ri % 0x100) & 0xff;
		return (crcH == data[len]) && (crcL == data[len+1]);
	}
	
	public static void fillCRC(int[] data,int len)
    {
		CRC16M crc16 = new CRC16M();
		crc16.update(data, len);
		int crc = crc16.getValue();
        data[len] = (crc >> 8) & 0xff;
        data[len+1] = (crc % 0x100) & 0xff;
    }
}

 ...
 * 上传文件后显示名称和链接
 */
function uploadifyMultipleFileComplete(file, data, response){
	var json = DWZ.jsonEval(data); 
	if (json.statusCode == DWZ.statusCode.ok) {
		//${uploadifyFileId}_hiddenId
		//${uploadifyFileId}_picId
		var _val=$('#'+this.settings.id+'_hiddenId').val()
		if(_val==''){
		  $('#'+this.settings.id+'_hiddenId').val(json.id);
		}
		else{
		   $('#'+this.settings.id+'_hiddenId').val(_val+','+json.id);
		} 
		 
		$('#'+this.settings.id+'_fileId').append(' <a  target="_blank" href="'+json.downloadUrl+'">'+json.name+'</a> ');
 ...
 * 删除已经上传的文件
 */
function uploadifyMultipleFileCompleteDelete(hiddenId,fileId){
     $('#'+hiddenId).val('');  
	 $('#'+fileId).html('');
}
